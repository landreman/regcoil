...
\param{theta\_transform\_refinement}
{integer}
{2}
{Only when \parlink{geometry\_option\_plasma} is 8.}
{\parlink{ntheta\_plasma} will be multiplied by this value
...
\param{zeta\_transform\_refinement}
{integer}
{2}
{Only when \parlink{geometry\_option\_plasma} is 8.}
{\parlink{nzeta\_plasma} will be multiplied by this value
when transforming from the original \vmec~double Fourier series representation to a single Fourier representation.
This parameter affects the time required to compute the single Fourier amplitudes,
but does not affect the time for other calculations.
}
...
{\ttfamily geometry\_option\_plasma} = 8: The plasma surface will be the last surface in the full radial grid of the \vmec~file specified by \parlink{wout\_filename}. This is then converted into a single Fourier representation. For more information, see \url{https://arxiv.org/abs/2011.03702}.\\

{\ttfamily geometry\_option\_plasma} = 9: The plasma surface in the single Fourier representation of option 8 and Bnorm information will be loaded from a text file, specified by \parlink{singleFourierFilename}.
...
\param{singleFourierFilename}
{string}
{{\ttfamily ""}}
{Only when \parlink{geometry\_option\_plasma} is 8 (write to) or 9 (read from).}
{Name of the text file which will be used for the plasma surface single Fourier representation. If \parlink{geometry\_option\_plasma}=9, the plasma surface from
this file will be used as the plasma surface for \regcoil. If \parlink{geometry\_option\_plasma}=8, \regcoil~will save the single Fourier representation of the plasma surface it computes into a file with this name.}

\myhrule

\param{use\_arclength\_angle}
{logical}
{true}
{Only when \parlink{geometry\_option\_plasma} is 8.}
...
\param{arclength\_separation}
{real}
{0.2}
{Only when \parlink{geometry\_option\_plasma} is 8 and \parlink{use\_arclength\_angle} is true.}
{Offset distance of the uniformly offset surface used to compute the uniform arclength angle. If \parlink{geometry\_option\_coil}=5, it is helpful, though not necessary, to make this the same as \parlink{separation}.}
...
{\ttfamily geometry\_option\_coil} = 2: The coil surface will be computed by expanding the plasma surface uniformly by a distance \parlink{separation}. The expanded surface will be saved to a local file specified by \parlink{nescin\_filename}. \\
...
{\ttfamily geometry\_option\_coil} = 5: The coil surface will be computed by expanding the plasma surface uniformly by a distance \parlink{separation}. This is similar to option 2, except that it is to be used with  \parlink{geometry\_option\_plasma}=8 or 9, and it does not save a Fourier representation of the coil surface. 
...
{real}
{0.2}
{Only when \parlink{geometry\_option\_coil} is 2 or 5.}
...
{If true, derivatives of $\chi^2$, $\chi^2_B$, and $\chi^2_K$ are computed at fixed target (indicated by \parlink{target\_option}) rather than at fixed $\lambda$. When \parlink{sensitivity\_option} $< 5$ , this option must be used with \parlink{general\_option} $> 3$ and \parlink{target\_option} = \texttt{"chi2\_B"}, \texttt{"lp\_norm\_K"} or \texttt{"max\_K\_lse"}. When \parlink{sensitivity\_option} $=6$ , this option must be used with \parlink{general\_option} $> 3$ and \parlink{target\_option} = \texttt{"chi2\_K"} or \texttt{"lp\_norm\_K"}  with $p=2$ .}
...
{integer}
{1}
{Symmetry assumed when computing derivative with respect to coil or plasma geometry parameters. This does not need to be the same as \parlink{symmetry\_option}.}
{\\
