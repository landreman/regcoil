...
class MagneticFieldEvaluator:
    ...
    The full-torus source geometry and current-dependent moments are built
    once. Calling the evaluator therefore only performs the position-dependent
    part of the Biot--Savart sum. Instances are normally obtained from
    :meth:`Solution.prepare_magnetic_field` rather than constructed directly.
    """
    ...
    def __post_init__(self):
        # The fused compiled singleton kernel consumes row-major (nsource, 3)
        # arrays. In the normal from_solution path these calls return views;
        # direct construction with strided arrays pays the copy only once.
        object.__setattr__(
    ...
    @classmethod
    def from_solution(
        cls, solution: Solution, *, theta_stride=1, zeta_stride=1
    ):
        coil = solution.problem.coil
        ...
        for period in range(coil.nfp):
            ...
            K_period = _flatten_grid(K_period).T

            weighted_current = K_period * weights_one_period[:, None]
            source_blocks.append(source_period)
            weighted_current_blocks.append(weighted_current)
        ...
        # The retained points still form a uniform periodic trapezoidal grid,
        # with both cell widths enlarged by their respective strides.
    ...
    def __call__(self, points, *, chunk_size=_MAGNETIC_FIELD_CHUNK_SIZE):
        """Evaluate the surface-current magnetic field at ``points``.

        ``chunk_size`` bounds the main temporary to approximately
        ``8 * chunk_size * nsource`` bytes. The direct quadrature is singular
        on, and is not intended for evaluation very close to, the winding
        surface.
        """
        ...
        for start in range(0, len(points), chunk_size):
            ...
            if len(point_block) == 1:
                B[start] = self._evaluate_single(point_block[0])
                continue
            else:
                distance_squared = -2.0 * (
                    point_block @ self.source_points.T
                )
            ...
            # With q_j = |p-r_j|^-3, use
            # sum_j w_j K_j x (p-r_j) q_j
            # = (sum_j q_j w_j K_j) x p
            #   - sum_j q_j w_j (K_j x r_j).
        ...
        return B.reshape(original_shape)

    def _evaluate_single(self, point):
        """Fast path for the singleton calls made by ODE integrators."""
