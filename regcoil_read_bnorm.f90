subroutine regcoil_read_bnorm()
  ...
  use regcoil_variables, only: load_bnorm, bnorm_filename, ntheta_plasma, nzeta_plasma, &
       Bnormal_plasma_current, theta_plasma, zeta_plasma, nfp, curpol, verbose, &
       nbf, bfn, bfm, bfs, bfc
  use safe_open_mod
  use stel_constants
  use stel_kinds
  ...
  if (allocated(Bnormal_plasma_current)) deallocate(Bnormal_plasma_current)
  allocate(Bnormal_plasma_current(ntheta_plasma,nzeta_plasma),stat=iflag)
  if (iflag .ne. 0) stop 'regcoil_read_bnorm Allocation error!'

  Bnormal_plasma_current = 0

  if (.not. load_bnorm) then
     if (verbose) print *,"Not reading a bnorm file, so Bnormal_plasma_current arrays will all be 0."
     return
  end if
  ...
  if ( nbf > 0 ) then
     ...
     do izeta = 1,nzeta_plasma
        do itheta = 1,ntheta_plasma
           do i = 1, nbf
              Bnormal_plasma_current(itheta,izeta) = Bnormal_plasma_current(itheta,izeta) + &
  ...
  Bnormal_plasma_current = Bnormal_plasma_current * curpol

!!$  ! I'll use an explicit loop here so there is no ambiguity about the order of theta vs zeta in the 1D vector:
!!$  do izeta = 1,nzeta_plasma
!!$     do itheta = 1,ntheta_plasma
!!$        Bnormal_plasma_current_1D((izeta-1)*ntheta_plasma+itheta) = Bnormal_plasma_current(itheta,izeta)
!!$     end do
!!$  end do
!!$  ! The syntax in the next line might be faster? But I should check the order.
!!$  !Bnormal_plasma_current_1D = reshape(Bnormal_plasma_current, (/ntheta_plasma*nzeta_plasma/))
