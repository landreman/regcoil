...
{\ttfamily general\_option} = 1: Compute the current potential for a range of alpha.\\

{\ttfamily general\_option} = 2: Do not compute the current potential, but rather load the current potential computed
by NESCOIL in the file \parlink{nescout\_filename}, reporting the $\chi^2_B$ and $\chi^2_J$ for it.
}
