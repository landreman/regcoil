input\chapter{Input Parameters}
\label{ch:input}
...
If an adjoint equation is solved in \texttt{REGCOIL}, (\parlink{sensitivity\_option} $> 1$), then derivatives of $\chi^2_B$ and/or $\chi^2_K$ are computed with respect to the Fourier coefficients defining the winding surface. 
...
\item \parlink{geometry\_option\_coil} = 3 
    ...
    \end{itemize}
\item \parlink{geometry\_option\_plasma} = 3, 4, or 5.
\item \parlink{sensitivity\_option} $>1$ denotes an adjoint solve must be performed. If $\chi^2_B$ and/or $\chi^2_K$ are included in the objective function, \parlink{sensitivity\_option} must be $>2$.
\item \parlink{nmax\_sensitivity} should be set to the largest value of $n$ that should be varied in the \texttt{NESCIN} file.
\item \parlink{nmax\_sensitivity} should be set to the largest value of $m$ that should be varied in the \texttt{NESCIN} file. 
...
\item If the coil-plasma distance is to be included in the objective function or constraints, then \parlink{coil\_plasma\_dist\_lse\_p} should be set to the desired value for the log-sum-exponent approximation. 
\item If the gradients of $\chi^2_B$ and $\chi^2_K$ are to be computed at fixed $\norm{\bm{K}}_p$ (rather than at fixed $\lambda$), \parlink{sensitivity\_option} should be $>1$. The following parameters matter when \parlink{sensitivity\_option} $>1$. 
	\begin{itemize}
