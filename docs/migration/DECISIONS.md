...
- **Decision:** TBD during Phase 2/4 spike (prefer meson-python or scikit-build-core; avoid custom makefile hooks if possible).
- **Consequences:** Phase 2 scaffold should pick one backend so CI and local dev share one path.
...
- **Options:**
  1. **pytest + Python extension** — call Fortran through the pyregcoil extension with small manufactured inputs; assert residuals/norms. No new Fortran test dep. Couples unit tests to bindings existing (Phase 4+).
