...
{integer}
{1}
{Typically will be used with \parlink{target\_option} = 3 to determine a sufficient value for $p$.}
{
...
for a range of $p$. The norm defined with and without the surface integral, \texttt{L\_p\_diagnostic\_with\_area} and \texttt{L\_p\_diagnostic}, are computed (see \parlink{target\_option} documentation for definitions).
}
